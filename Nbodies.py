...
scene2 = canvas(title='N bodies simulator', x=0, y=0, width=800,
                height=800, center=vector(0, 0, 0))  # background=vector(1, 1, 1
G = 6.67408e-11
collided = False
planets = []
...
class Planets(object):
    def __init__(self, xo, yo, zo, vox, voy, voz, mass, radius):
        ...
        self.shape = simple_sphere(pos=vector(
            xo, yo, zo), radius=radius, color=vector(0, 1, 0))
...
def create_planet(x, y, z, vx, vy, vz, m, d):
    ...
    planets.append(Planets(xo=x, yo=y, zo=z, vox=vx,
                           voy=vy, voz=vz, mass=m, radius=r))
...
def mergingStars(p1, p2):
    if p1.mass > p2.mass:
        ...
        p1.velocity[2] = (p1.mass * p1.velocity[2] + p2.mass *
                          p2.velocity[2]) / (p1.mass + p2.mass)
...
def calca(target, kind, data):
    ...
    for planet in planets:
        if planet != target:
            if kind == 0:
                distance = math.sqrt((planet.pos[0] - data[0])**2 + (
                    planet.pos[1] - target.pos[1])**2 + (planet.pos[2] - target.pos[2])**2)
            if kind == 1:
                distance = math.sqrt((planet.pos[0] - target.pos[0])**2 + (
                    planet.pos[1] - data[1])**2 + (planet.pos[2] - target.pos[2])**2)
            if kind == 2:
                distance = math.sqrt((planet.pos[0] - target.pos[0])**2 + (
                    planet.pos[1] - target.pos[1])**2 + (planet.pos[2] - data[2])**2)
...
def ODE(planet, func, index, dt):
    ...
    data[index] = planet.pos[index] + \
        planet.velocity[index] * dt / 2 + 0.5 * k1 * (dt / 2)**2
    k2 = func(planet, index, data)
    data[index] = planet.pos[index] + \
        planet.velocity[index] * dt / 2 + 0.5 * k2 * (dt / 2)**2
    k3 = func(planet, index, data)
    data[index] = planet.pos[index] + \
        planet.velocity[index] * dt + 0.5 * k3 * dt**2
    k4 = func(planet, index, data)
    a = (1 / 6) * (k1 + 2 * k2 + 2 * k3 + k4)
    planet.pos[index] = planet.pos[index] + \
        planet.velocity[index] * dt + 0.5 * a * dt ** 2
...
while not collided:
    rate(60)
    for planet in planets:
        planet.shape.pos = vector(ODE(planet, calca, 0, dt), ODE(
            planet, calca, 1, dt), ODE(planet, calca, 2, dt))
    t = t + dt
