...
scene2 = canvas(title='N bodies simulator', x=0, y=0, width=800, height=800, center=vector(2500e6, 2500e6, 0)) # background=vector(1, 1, 1
G = 6.67408e-11
collided = False
planets = []
...
def create_planet(x, y, vx, vy, m, d):
    ...
    planets.append(Planets(xo=x, yo=y, vox=vx, voy=vy, mass=m, radius=r))
...
def calca(target, kind, data):
    ...
    for planet in planets:
        if planet != target:
            if kind == 0:
                distance = math.sqrt((planet.pos[0] - data[0])**2 + (planet.pos[1] - target.pos[1])**2)
            else:
                distance = math.sqrt((planet.pos[0] - target.pos[0])**2 + (planet.pos[1] - data[1])**2)
